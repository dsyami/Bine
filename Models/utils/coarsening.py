...
def metis(W, levels, rid=None):
    ...
    N, N = W.shape
    if rid is None:
        rid = np.random.permutation(range(N))   # N节点的随机排序
    parents = []
    ...
    for _ in range(levels):
        ...
        idx_row, idx_col, val = scipy.sparse.find(W)    # 返回矩阵的非零元素的索引和值(row, col, val)
        perm = np.argsort(idx_row)      # np.argsort将元素从小到大排列，提取其在排列前对应的index(索引)输出。
        rr = idx_row[perm]
        cc = idx_col[perm]
        vv = val[perm]
        cluster_id = metis_one_level(rr, cc, vv, rid, weights)  # rr is ordered 节点聚合表
        ...
        W = scipy.sparse.csr_matrix((nvv, (nrr, ncc)), shape=(Nnew, Nnew))      # 相同位置节点的值加和
        W.eliminate_zeros()
...
def metis_one_level(rr, cc, vv, rid, weights):
    ...
    nnz = rr.shape[0]   # col的数量
    N = rr[nnz-1] + 1   # 节点数量

    marked = np.zeros(N, np.bool_)
    rowstart = np.zeros(N, np.int32)    # 统计每个节点的邻接点对应的row开始的标记row[rowstart]
    rowlength = np.zeros(N, np.int32)   # 统计每个节点的邻接点对应的row数量
    cluster_id = np.zeros(N, np.int32)

    oldval = rr[0]  # 从第0个col开始
    count = 0
    clustercount = 0
    ...
    for ii in range(N):
        tid = rid[ii]       # 从随机节点tid开始遍历
        if not marked[tid]:
            wmax = 0.0
            rs = rowstart[tid]
            marked[tid] = True
            bestneighbor = -1
            for jj in range(rowlength[tid]):    # 遍历tid节点的所有邻接点
                nid = cc[rs+jj]     
            ...
            clustercount += 1               # 将需要聚合的两个节点按聚合顺序标记为同一个数clustercount
    return cluster_id
