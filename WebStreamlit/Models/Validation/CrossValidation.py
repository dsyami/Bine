...
import numpy as np
import Network.lib_for_GCN.util.models_utils as models_utils
import pickle
...
class CrossValidation:
    ...
    def cross_validation_kfold(self, args, DIR, SubDataLoader, Model, folds=10):
        ...
        try:
            os.makedirs(f"./Saved_Files/{modelname}/")
            print("save files makedirs success!")
        except OSError:
            print("save files makedirs failed.")
            pass
        # 保存args为可读性txt和pkl字典
        with open(f'./Saved_Files/{modelname}/args.txt', 'w') as f:
            f.write(f'{modelname}\n')
            for key, value in vars(args).items():
                f.write(f'{key}: {value}\n')
        with open(f'./Saved_Files/{modelname}/args.pkl', 'wb') as f:
            print("save pkl")
        ...
        for f, fold in enumerate(kf.split(subdataloader)):
            ...
            print(f"第 {f} 次交叉验证---------------------------------------")
            maxloss, maxacc = self.run_model(args, TrainLoader, TestLoader, optimizer, model, modelname, f)
        ...
        np.save(f'./Saved_Files/{modelname}/loss_list.npy', loss_list)
        np.save(f'./Saved_Files/{modelname}/acc_list.npy', acc_list)
    ...
    def run_model(self, args, TrainLoader, TestLoader, optimizer, model, modelname, fold):
            # 初始化loss和acc列表
            max_acc = 0
            max_loss = 1e5
            max_epo = 0
            ...
            for epoch in range(args.epochs):
                ...
                if epoch % 20 == 0:
                    try:
                        os.makedirs(f'./Saved_Files/{modelname}/{fold}fold/{epoch}/')
                        print("makedirs success")
                    except OSError:
                        print("makedirs failed")
                        pass
                    torch.save(model.state_dict(), f'./Saved_Files/{modelname}/{fold}fold/{epoch}/model_{epoch}.pth')
                    np.save(f'./Saved_Files/{modelname}/{fold}fold/{epoch}/{modelname}_train_loss_epoch{epoch}.npy', train_loss_epoch)
                    np.save(f'./Saved_Files/{modelname}/{fold}fold/{epoch}/{modelname}_train_acc_epoch{epoch}.npy', train_acc_epoch)
                    np.save(f'./Saved_Files/{modelname}/{fold}fold/{epoch}/{modelname}_test_loss_epoch{epoch}.npy', test_loss_epoch)
                    np.save(f'./Saved_Files/{modelname}/{fold}fold/{epoch}/{modelname}_test_acc_epoch{epoch}.npy', test_acc_epoch)
